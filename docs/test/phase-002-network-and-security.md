...
This procedure verifies `P002-TASK-002`, `P002-TASK-003`, and the network portions of `P002-TASK-011` through `P002-TASK-014`. It covers the complete 24 action launch registry even though only seven routes are available in Phase 002.
...
The full test run must report 52 passed and 0 failed. The relevant named cases are:

| Test | Coverage |
| --- | --- |
| `P002-TEST-001-action-and-error-registries` | Registry and error versions, 24 unique actions, frozen schemas, available handler coverage, and 43 declared error codes. |
