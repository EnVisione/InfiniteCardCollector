...
| OBSERVED | Launch content boundary | Launch contains exactly three Deck Sets, 156 base identities, four Editions, 624 permanent forms, eleven Grades, eleven Power Tiers, twelve Traits, twenty-four Jokers, twelve Arcana, and twelve formations. | `DEC-003`, `CORE-REQ-002`, `docs/general/content-catalog.md`, `docs/implementation/launch-foundation.md` | Invalidated by an authorized change to any cited protected source or its pinned digest. |
...
- The complete protected plan set and saved goal pass their recorded digest checks with `CORE-PHASE-001` as the next authorized phase.
...
- Pinned copies or digests of `docs/general/content-catalog.md`, `docs/general/product-vision.md`, `docs/implementation/launch-foundation.md`, `DEC-003`, `DEC-006`, and `DEC-010` match the phase intake. Any mismatch stops implementation for plan conflict review.
...
|---|---|---|---|---|
| Protected plan set and saved goal | Plan Creator and Goal Creator | Digests match; active work authorizes `CORE-PHASE-001`; scope is unchanged. | Recompute and compare every recorded digest before editing implementation. | Stop with `PLAN_REVISION_CONFLICT`; do not update a digest or rebind the goal. |
...
| Phase 001 completion packet | `CORE-PHASE-002`, verification system, repository operations | Merged artifact identities, commands, tests, traces, reports, docs, review, merge, and tag evidence are complete and internally consistent. | Any relevant source, version, schema, configuration, tool, or artifact change invalidates affected evidence. | Packet audit and protected-plan digest comparison. |

## Work Packages

| Task ID | Requirement IDs | Work | Inputs and dependencies | Outputs | Affected components or interfaces | Verification |
|---|---|---|---|---|---|---|
| P001-TASK-001 | `CORE-REQ-002`, `CORE-REQ-003`, `CORE-REQ-013` | Verify phase entry and define the typed schema, catalog and configuration version fields, stable-ID grammar, key compatibility rules, and immutable calculation input and output boundaries. | Phase 000 packet, protected plan digests, `DEC-003`, `DEC-006`, `DEC-010` | Entry record and shared domain contract | Shared Types, Catalogs, Config, Math, test fixtures | Type checks, version mismatch tests, key round trips, protected-plan digest proof |
...
- `P001-TASK-001` is strictly first. A failed digest, Phase 000 integration, tool, or source-contract check stops the phase. Recovery is to restore the verified input or obtain an authorized plan revision; no downstream work may proceed.
...
| Protected plan artifact changes | Pre-entry and pre-merge digest checks. | Expected and observed digest mismatch. | Stop with `PLAN_REVISION_CONFLICT`; do not overwrite or rebind. | Entire phase authorization and completion packet. | Resume only after owner-authorized plan handling and a new valid handoff. |
...
- Issue, milestone, roadmap Project, pull request, review, check, merge, tag, and wiki-preparation state consistent with the merged repository, plus proof that every protected plan artifact and saved goal retained its pinned digest.
