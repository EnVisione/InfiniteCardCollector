...
The repository suite passes 64 tests with 0 failures. The `P003-TEST-008-deckset-pack-service` fixture covers the five card result, price deduction, unique slots, configured Tier roll count, pity state, pack provenance, classifications, profile validation, duplicate operation rejection, insufficient currency no mutation, and Radioactive identity selection. The `P003-TEST-009-offer-pack-service` fixture covers disclosed Joker rarity and Arcana item odds, three unique saved offers, persistent Joker pity, invalid choice no mutation, exact one-choice Joker and Arcana grants, claimed pending projection, profile validation, and replay rejection. The client reveal state machine is covered separately by [the pack reveal evidence](phase-003-pack-reveal.md). Grade, Trait, and Arcana progression are covered by [the progression evidence](phase-003-progression.md).
